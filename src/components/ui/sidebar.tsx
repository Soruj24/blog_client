...
function useIsMobile() {
  ...
  React.useEffect(() => {
    ...
    // Set initial value via callback to avoid synchronous setState
    onChange();
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "p-2 text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground rounded-md",
        className
      )}
      onClick={(event) => {
        onClick?.(event);
...
>(({ className, showIcon = false, ...props }, ref) => {
  const width = React.useMemo(() => {
    // Use a deterministic width to avoid Math.random in render
    return "60%";
  }, []);
